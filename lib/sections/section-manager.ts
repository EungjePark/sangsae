...
🎯 결과물: 강력한 USP 문장 + 3-4개의 구조화된 핵심 특징 불렛 포인트`;
    ...
    case 'main_feature':
      instruction += `
## 작업 목표: ${productData.name}의 핵심 기능과 차별화된 장점을 매력적으로 설명

## 글쓰기 형식 가이드라인
- 가장 인상적인 기능부터 우선순위에 따라 작성
- 각 기능은 구체적인 혜택과 연결하여 서술
- 감각적 묘사와 구체적 수치를 활용한 설득력 있는 표현
- 쉽게 스캔 읽기가 가능하도록 구조화된 형식 (제목, 소제목, 불릿포인트)
- 기능 이름은 볼드체로 강조하고 2-3문장으로 상세 설명

## 설득 구조 활용
- 문제 제기와 해결 방안(PS 구조): 고객의 문제/고민을 제시하고 제품이 해결하는 방식 설명
- 가치 입증: 각 기능의 품질과 효과를 뒷받침하는 구체적 증거 제시
- 사회적 증거: 각 기능에 대한 다른 사용자들의 긍정적 경험 포함
- 비교: 다른 유사 제품과의 간접적 차별점 언급 (직접적 비교는 피함)

## 필수 포함 요소
1. **핵심 가치 요약문** - 제품이 해결하는 핵심 문제나 제공하는 가치 (1-2문장)
2. **주요 기능 및 장점 3-5개** - 각 기능별로:
   • 기능 이름/특징 (볼드체)
   • 작동 방식 간략 설명
   • 사용자에게 제공하는 구체적 혜택
   • 실제 사용 시나리오나 예시
   • 사용자 경험이나 후기 (각 기능당 1개 이상)
3. **차별화 포인트** - 경쟁 제품과 비교했을 때 우수한 점 (직접적 비교는 피하고 암시적으로)
4. **품질/성능 증명 요소** - 내구성, 테스트 결과, 품질 보증 등

## 특별 주의사항
- 다음 키워드를 자연스럽게 통합: ${Array.isArray(productData.keywords) ? productData.keywords.join(", ") : productData.keywords || ""}
- 다음 특징에 특별히 집중: ${productData.mainBenefits || productData.features || '제품의 주요 특징'}
- 전문 용어와 일상 언어를 적절히 균형
- 과장된 표현보다 구체적인 증거와 예시로 설득
- 감각적 표현(시각, 촉각, 청각 등)을 활용한 생생한 묘사
- 가격 정보는 포함하지 말고 제품의 가치와 혜택에 집중

🎯 최종 결과물: 설득력 있는 기능 및 장점 설명 (3-5개 단락, 각 단락은 핵심 기능 중심, 사용자 경험 포함)`;
      break;
...
🎯 결과물: 체계적으로 구조화된 제품 정보 + 핵심 사양의 명확한 표현`;
    ...
    case 'how_to_use':
      instruction += `
## 작업 목표: ${productData.name}의 사용 방법을 명확하고 단계적으로 안내

## 글쓰기 형식 가이드라인
- 명확한 단계별 지시문 (번호 매기기 사용)
- 각 단계는 간결하고 직접적인 문장으로 구성
- 복잡한 단계는 하위 단계로 세분화
- 시각적으로 진행 순서를 상상할 수 있는 서술
- 전문 용어는 간단한 설명 추가

## 필수 포함 요소
1. **사용 목적 소개** - 제품을 통해 얻을 수 있는 결과 (1-2문장)
2. **준비 사항** - 사용 전 필요한 준비물이나 환경 설정
3. **단계별 사용법** - 최소 3단계, 최대 7단계의 명확한 지시:
   • 1단계: [행동] - 부연설명
   • 2단계: [행동] - 부연설명
   • (이하 계속)
4. **전문가 팁** - 최적의 결과를 얻기 위한 3-5가지 추가 팁
5. **주의사항** - 흔한 실수나 피해야 할 사항들

## 특별 주의사항
- 예시나 구체적 시나리오를 포함하여 이해도 높이기
- ${productData.category} 제품의 특성에 맞는 특별 사용 팁 포함
- 초보자도 쉽게 따라할 수 있도록 설명
- 시간 관련 정보 (얼마나 걸리는지, 얼마나 기다려야 하는지 등) 포함
- 시각적 단서나 감각적 피드백 설명 (예: "클릭음이 들릴 때까지")

🎯 최종 결과물: 초보자도 쉽게 따라할 수 있는 단계별 사용 안내서`;
      break;
...
🎯 결과물: 계층화된 혜택 구조 + 구체적 결과 설명 + 감정적/실용적 가치 연결`;
    ...
    case 'faq':
      instruction += `
1. [신뢰+행동+반박 제거] ${productData.name}에 관한 가장 중요한 질문과 명확한 답변을 제공하세요.
2. 다음 주제 영역에서 5-7개의 질문을 선정하여 균형 있게 다루세요:
   • 제품 특성/기능 관련 질문 (1-2개)
   • 사용/관리 방법 관련 질문 (1-2개)
   • 구매 결정에 영향을 미치는 불확실성 해소 질문 (1-2개)
   • 기대 효과 및 사용자 경험 관련 질문 (1-2개)
   • 경쟁 제품과의 차별점에 관한 질문 (1개)
3. 질문은 실제 고객이 물을 법한 자연스러운 형태로 작성하세요.
4. 답변은 간결하면서도 완전하게, 실질적인 정보를 제공하세요.
5. 각 답변에는 최소 1개 이상의 실제 사용자 경험이나 후기를 포함하세요. (예: "많은 고객들이 ~했다고 합니다", "실제 사용자 A씨는 ~라고 후기를 남겼습니다")
6. 구매를 망설이는 고객의 우려를 해소할 수 있는 질문을 반드시 포함하세요.
7. 가격 관련 정보는 포함하지 말고, 대신 제품의 가치와 혜택을 강조하세요.

🎯 결과물: 균형 잡힌 5-7개의 Q&A 세트 + 실제 사용자 경험이 포함된 답변 + 구매 우려 해소 요소`;
      break;
